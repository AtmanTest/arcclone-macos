/* TeamAI Changelog */
const ChangelogData = [
  {v:"0.21.0", date:"2026-07-29", commit:"v0.21.0", author:"Kimi3", items:[
    "25 providers: ajout Copilot, Perplexity, Mistral, DeepSeek, Meta AI, Qwen, HuggingChat, Phind, You.com, Poe, Groq, Cohere, Pi.ai, OpenRouter, LMSYS Arena",
    "ChatGPT: suppression doublon, garder uniquement ChatGPT o3 (gratuit)",
    "Ajouter IA: modal propre (nom + URL + ic\u00f4ne) \u2192 redirige vers assistant connexion",
    "MES IA: bouton supprimer avec confirmation modale",
    "Export / Import providers JSON",
    "fix: version badge affiche maintenant la bonne version (via version.json)",
    "fingerprints dispatch pour les 25 providers",
  ]},
  {v:"0.20.0", date:"2026-07-29", commit:"7da9089", author:"Kimi3", items:[
    "fix Grok: input contenteditable + send selector exact + fallback curseur",
    "fix Popup mise \u00e0 jour: commit SHA + auteur + correctifs + Valider/Annuler",
    "fix Kimi URL: kimi.moonshot.cn \u2192 kimi.ai",
    "fix GLM + Kimi: submit via Enter React-compatible",
  ]},
  {v:"0.19.1", date:"2026-07-29", commit:"88486d4", author:"Hermes", items:["Sync version + AGENT_LOG"]},
  {v:"0.19.0", date:"2026-07-29", commit:"fdebfc5", author:"Kimi3", items:["fix: fingerprints hostname + execCommand bypass isTrusted + button scoring"]},
  {v:"0.7.0", date:"2026-07-27", items:["Session persistante", "Reset layout", "Navigation buttons", "Bookmarks", "File upload", "Z GLM ?lang=en"]},
];
...
const Changelog = {
  open(updateInfo) {
    ...
    if (updateInfo && updateInfo.commit) {
      ...
      `;
      banner.querySelector('#update-confirm').addEventListener('click', () => { if (updateInfo.onConfirm) updateInfo.onConfirm(); modal.classList.remove('open'); });
      banner.querySelector('#update-cancel').addEventListener('click', () => { if (updateInfo.onCancel) updateInfo.onCancel(); banner.style.display = 'none'; });
    } else {
